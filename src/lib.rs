...
//! 零依赖: GBK 编解码走 Win32 CP936 直通 FFI, 不引 encoding_rs。
//! 来源: danqing-log `src/encoding.rs` (2026-09-09 下沉, 簇F/F2), 2026-09-10 从 danqing 独立为兄弟 crate。
...
/// 非 Windows 平台降级 lossy (此处仅为可编译, 产品边界 Windows-only)。
#[cfg(windows)]
mod gbk {
    use std::ptr;
